...
fn main() -> std::io::Result<()> {
    ...
    while let Some(Ok(line)) = lines.peek() {
        ...
        let line = lines.next().unwrap()?; // safe unwrap
    ...
    // Now process alignment lines one by one
    for line in lines {
        let line = line?;
        process_record(&line, &ref_names, &mut bgzf_writer)?;
    }

    // Write BAM EOF marker
...
fn process_record(line: &str, ref_names: &[String], bgzf_writer: &mut GzEncoder<File>) -> std::io::Result<()> {
    let fields: Vec<&str> = line.split('\t').collect();
    if fields.len() < 11 {
        return Ok(()); // skip malformed
    }
    ...
    record.extend_from_slice(&[0u8; 4]); // placeholder for block size
    ...
    record.extend_from_slice(&encode_cigar(cigar));
    record.extend_from_slice(&encode_seq(seq));
    record.extend_from_slice(&encode_qual(qual));
